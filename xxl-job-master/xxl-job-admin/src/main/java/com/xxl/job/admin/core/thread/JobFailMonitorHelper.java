...
import com.xxl.job.admin.core.util.MessageUtil;
						...
						if (jobLogId != null && jobLogId > 0) {
							...
							if (log!=null) {
								...
								if (ReturnT.FAIL_CODE == log.getTriggerCode()|| ReturnT.FAIL_CODE==log.getHandleCode()) {
									...
									if(info!=null && info.getPhone()!=null && info.getPhone().trim().length()>0){
										Set<String> phonelSet = new HashSet<String>(Arrays.asList(info.getPhone().split(",")));
										for (String phone: phonelSet) {
										XxlJobGroup group = XxlJobDynamicScheduler.xxlJobGroupDao.load(Integer.valueOf(info.getJobGroup()));
										String content = MessageFormat.format("任务调度失败, 执行器名称:{0}, 任务描述:{1},任务调度时间:{2}", group!=null?group.getTitle():"null", info.getJobDesc(),time);
										content=content.replaceAll(",","%2C").replaceAll(" ", "%20");
										String string = MessageUtil.sendMessage(phone,content);
										logger.info("短信发送日志结果:"+string);
										}
									}
								}
							}
