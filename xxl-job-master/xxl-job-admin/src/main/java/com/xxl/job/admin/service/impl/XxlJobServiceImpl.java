...
import com.xxl.job.admin.core.util.PhoneFormatCheckUtils;
...
public class XxlJobServiceImpl implements IXxlJobService {
	...
	public ReturnT<String> add(XxlJobInfo jobInfo) {
		...
		if (StringUtils.isNotBlank(jobInfo.getPhone())) {
			boolean bl=true;
			String[] phones = jobInfo.getPhone().split(",");
			for (String phone: phones) {
				 bl=PhoneFormatCheckUtils.isChinaPhoneLegal(phone);
				 if(!bl){
						return new ReturnT<String>(ReturnT.FAIL_CODE, "请输入正确的手机号吗!");
					}
			}
		}
	...
	public ReturnT<String> reschedule(XxlJobInfo jobInfo) {
		...
		exists_jobInfo.setPhone(jobInfo.getPhone());
